...
probabilidad_prediccion = model.predict_proba(nuevo_dato)[:, 1]
if model.predict(nuevo_dato)[0] == 1:
    print(f'El nuevo dato es una setosa con una probabilidad de {probabilidad_prediccion[0]:.2f}')
else:
    print(f'El nuevo dato no es una setosa con una probabilidad de {1 - probabilidad_prediccion[0]:.2f}')
