...
# Seleccionar las características 'aspiration' y 'compression-ratio'
x = df[['aspiration', 'compression-ratio']]
y = df['fuel-type']

# Codificar la columna 'aspiration' como variables dummy (one-hot)
x = pd.get_dummies(x, columns=['aspiration'], drop_first=True)

# Normalizar la característica 'compression-ratio'
scaler = StandardScaler()
x['compression-ratio'] = scaler.fit_transform(x[['compression-ratio']])
